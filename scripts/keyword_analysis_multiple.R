...
data <- data.frame(KW,WEOG_index,a_index,u_index,m_index,o_index)
head(data)
...
head(x5)
g5a<-ggplot(data = x5[FROM:TO,], aes(x = reorder(KW,prop),y=prop-.5, label = Freq)) + 
  geom_col(fill='lightblue4',color='grey10') +
  geom_text(nudge_y = .02)+
  coord_flip() +
  theme(text = element_text(size=16)) +
...
Ga<-cowplot::plot_grid(g1a,g2a,g3a,g4a)
Ga
ggsave(filename = 'keyword_combination_ver1.pdf',Ga,device = 'pdf',width = 14,height = 10)
